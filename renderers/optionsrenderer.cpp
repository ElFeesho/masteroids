...
void OptionsRenderer::render(Gfx &gfx, Position &, Shape &, Direction &, RGB &) const
{
    ...
    gfx.drawText(false, textPosition, 80 + textHeight * 8, stringBuffer.str(), menu_sel == LIVES ? RGB::YELLOW
																															: RGB::WHITE, TextAlignment::CENTRE);

	stringBuffer.str("");
	stringBuffer << "Difficulty: " << difficultyLabels[Options::difficulty];
    gfx.drawText(false, textPosition, 80 + textHeight * 9, stringBuffer.str(), menu_sel == DIFFICULTY ? RGB::YELLOW
																																  : RGB::WHITE, TextAlignment::CENTRE);

	stringBuffer.str("");
	stringBuffer << "Team Killing: " << (Options::team_kill == 0 ? "Off" : "On");
    gfx.drawText(false, textPosition, 80 + textHeight * 10, stringBuffer.str(), menu_sel == TEAM_PLAY ? RGB::YELLOW
																																 : RGB::WHITE, TextAlignment::CENTRE);

	stringBuffer.str("");
	stringBuffer << "Players: " << Options::players;
    gfx.drawText(false, textPosition, 80 + textHeight * 11, stringBuffer.str(), menu_sel == PLAYER_COUNT ? RGB::YELLOW
																																	: RGB::WHITE, TextAlignment::CENTRE);

	stringBuffer.str("");
	stringBuffer << "Max Bullets: " << Options::max_bullets;
    gfx.drawText(false, textPosition, 80 + textHeight * 12, stringBuffer.str(), menu_sel == MAX_BULLETS ? RGB::YELLOW
																																   : RGB::WHITE, TextAlignment::CENTRE);

	stringBuffer.str("");
	stringBuffer << "Music: " << (Options::music ? "Yes" : "No");
    gfx.drawText(false, textPosition, 80 + textHeight * 13, stringBuffer.str(), menu_sel == MUSIC ? RGB::YELLOW
																															 : RGB::WHITE, TextAlignment::CENTRE);

    gfx.drawText(false, textPosition, 80 + textHeight * 14, "Configure Controls", menu_sel == CONTROLS ? RGB::YELLOW
																																  : RGB::WHITE, TextAlignment::CENTRE);
    gfx.drawText(false, textPosition, 80 + textHeight * 15, "Start to Exit", RGB::WHITE, TextAlignment::CENTRE);
}
