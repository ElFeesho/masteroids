	...
	void render(GfxWrapper* gfx, Position& position, Shape& shape, Direction& direction)
	{
		for(int i = 0;i<6;i++)
		{
			if(i!=5)
			{
				...
				gfx->drawLine(position.X()+cos(rot1)*peaks[i], 
								position.Y()+sin(rot1)*peaks[i], 
								position.X()+cos(rot2)*peaks[i+1], 
								position.Y()+sin(rot2)*peaks[i+1], RGB::white);
			}
			else
			{
				...
				gfx->drawLine(position.X()+cos(rot1)*peaks[i], 
								position.Y()+sin(rot1)*peaks[i], 
								position.X()+cos(rot2)*peaks[0], 
								position.Y()+sin(rot2)*peaks[0], RGB::white);
			}
		}
	}
